...
int countBTNodes(BinaryTreeNode<int>* root){
    if(root==NULL){
        return 0;
    }
    ...
    int leftSum = countBTNodes(root->left);
    int rightSum = countBTNodes(root->right);

    count= count+ leftSum+rightSum;
    return count;
}
...
int main(){
    ...
    cout<<endl;
    printTree(root);
    cout<<"\nTotal Nodes in BT:"<<countBTNodes(root);
    delete root;
    return 0;
}